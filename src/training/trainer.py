...
class PlantTrainer:
    ...
    def _validate_and_convert_config(self, config: Dict) -> Dict:
        ...
        train_transforms['RandomHorizontalFlip'] = float(train_transforms['RandomHorizontalFlip'])
        train_transforms['RandomVerticalFlip'] = float(train_transforms['RandomVerticalFlip'])
    ...
    def train_epoch(self, epoch: int) -> Dict[str, float]:
        ...
        logging.info(f'Train Epoch {epoch} - Accuracy: {metrics["accuracy"]:.4f}, '
    ...
    def train(self) -> Dict[str, Any]:
        ...
        for epoch in range(1, epochs + 1):
            ...
            if val_metrics['macro_f1'] > self.best_val_f1:
                ...
                logging.info(f"✅ Saved best model with Macro-F1: {val_metrics['macro_f1']:.4f}")
            else:
                self.patience_counter += 1
                logging.info(f"⏳ No improvement. Patience counter: {self.patience_counter}/{patience}")
    ...
    def _save_metrics(self):
        ...
        metrics_path = os.path.join(self.config['output']['model_dir'], 'training_metrics.csv')
    ...
    def mock_training_step(self, batch_size: int = 2) -> Dict[str, float]:
        ...
        mock_target = torch.randint(0, len(self.class_names), (batch_size,)).to(self.device)
